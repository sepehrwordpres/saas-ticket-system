<x-app-layout>
    <div class="max-w-xl mx-auto py-12 px-6">
        <div class="mb-8">
            <h2 class="text-sm font-bold text-white">ویرایش دپارتمان: {{ $department->title['fa'] ?? '' }}</h2>
            <p class="text-[11px] text-slate-400 mt-1">عنوان و شناسه یکتای این بخش سازمانی را به‌روزرسانی کنید.</p>
        </div>
        ...
        <form action="{{ route('admin.departments.update', $department->id) }}" method="POST" class="space-y-5 bg-slate-900/50 border border-slate-800 rounded-xl p-6 shadow-xl">
            ...
            <div>
                <label for="title_fa" class="block text-xs font-medium text-slate-400 mb-2">عنوان دپارتمان (فارسی):</label>
            ...
            <div>
                <label for="title_en" class="block text-xs font-medium text-slate-400 mb-2">عنوان دپارتمان (English):</label>
            ...
            <div>
                <label for="slug" class="block text-xs font-medium text-slate-400 mb-2">شناسه یکتا در URL (Slug):</label>
                <input type="text" name="slug" id="slug" value="{{ old('slug', $department->slug) }}" required dir="ltr"
                       class="w-full bg-slate-950/50 border border-slate-800 rounded-xl px-4 py-3 text-slate-200 placeholder-slate-600 focus:outline-none focus:border-amber-500 text-xs font-mono transition-all">
                <p class="text-[10px] text-slate-500 mt-1.5">از اسلاگ برای ساختن مسیرهای تمیز استفاده می‌شود (مانند: `support`).</p>
                @error('slug')
                    <p class="text-rose-400 text-[11px] mt-1 px-1">{{ $message }}</p>
                @enderror
            </div>
