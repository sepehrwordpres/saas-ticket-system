<x-app-layout>
    <div class="max-w-xl mx-auto py-12 px-6">
        <div class="mb-8">
            <h2 class="text-sm font-bold text-white">ویرایش کارشناس: {{ $support->name }}</h2>
            <p class="text-[11px] text-slate-400 mt-1">مشخصات حساب کاربری و دپارتمان‌های مجاز این کارشناس را به‌روزرسانی کنید.</p>
        </div>
        ...
        <form action="{{ route('admin.supports.update', $support->id) }}" method="POST" class="space-y-5 bg-slate-900/50 border border-slate-800 rounded-xl p-6 shadow-xl">
            ...
            <div>
                <label for="name" class="block text-xs font-medium text-slate-400 mb-2">نام و نام خانوادگی:</label>
            ...
            <div>
                <label for="email" class="block text-xs font-medium text-slate-400 mb-2">آدرس ایمیل (نام کاربری):</label>
            ...
            <div class="pt-2 border-t border-slate-800/60">
                <label for="password" class="block text-xs font-medium text-amber-400 mb-1">رمز عبور جدید (اختیاری):</label>
                <p class="text-[10px] text-slate-500 mb-2">اگر نمی‌خواهید رمز عبور کارشناس تغییر کند، این بخش را خالی بگذارید.</p>
            ...
            <div>
                <label for="password_confirmation" class="block text-xs font-medium text-slate-400 mb-2">تکرار رمز عبور جدید:</label>
                <input type="password" name="password_confirmation" id="password_confirmation" dir="ltr"
                       class="w-full bg-slate-950/50 border border-slate-800 rounded-xl px-4 py-3 text-slate-200 placeholder-slate-600 focus:outline-none focus:border-amber-500 text-xs font-mono transition-all">
            </div>

            <div class="pt-4 border-t border-slate-800/60">
                <label class="block text-xs font-medium text-purple-400 mb-3">🏢 تعیین دپارتمان‌های مجاز کارشناس:</label>
